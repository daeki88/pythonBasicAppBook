...
이름이 없는 간단한 함수를 만들 때 `lambda` 예약어를 이용한다. 사용방법은 `lambda 매개변수 : 반환될 수식`와 같다.
...
- 리스트의 sort 메소드는 key 인자값으로 하나의 매개변수만을 갖는 함수를 취한다. 이 함수의 반환값들을 가지고 `<` 연산을 하여 리스트 항목을 정렬한다.
...
파이썬은 문서화 문자열(documentation string)이라고 불리우는, 줄여서 docstrings라 불리우는 편리한 기능을 가지고 있다. docstring은 프로그램을 알아보기 쉽게 해 주고, 또 나중에 프로그램에 대한 설명서를 작성할 때 유용하게 사용될 수 있는 중요한 도구이다. 아래 예제와 같이, docstring은 프로그램이 실행 중일 때도 읽어올 수 있다.
...
- 문서화 문자열은 함수이름 선언 바로 다음 줄에서 따옴표 3개를 사용한 문자열을 이용한다.
- 첫 줄은 보통 한 줄로 적으며 마침표로 끝낸다.
- 둘째 줄은 빈 줄로 놓는다.
- 셋째 줄부터 필요한 설명들을 적는다.
- 문서화 문자열이 끝난 다음 줄부터 함수의 몸통이 시작된다.

## 연습

1. 별의 시작 위치 `x`, `y`와 한 변의 길이를 매개변수로 하는 별 그리는 함수를 작성하고 실행하시오. 함수의 이름과 매개변수를 `별그리기(x, y, 한변)`로 하시오.
1. 위의 `별그리기(옆, 앞뒤, 한변)`함수를 이용하여 별의 시작 위치및 한 변의 길이를 무작위 수를 생성하여 10개를 그리시오. 반복문을 이용하시오.
...
1. 거북이를 이용하여 창을 클릭한 곳에 별이 그려지는 프로그램을 작성하시오. 위에서 작성한 `별그리기` 함수를 수정해서 사용하시오.
