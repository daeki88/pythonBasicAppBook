...
이 때는 `"What's your name?"`과 같이 큰 따옴표를 사용해도 되지만 탈출 문자 `\`를 이용해서 작은 따옴표를 사용하면서도 문자열 안에 작은 따옴표 `'`를 넣을 수 있다. 즉, `\'`와 같이 작은 따옴표 앞에 역슬래스 `\`를 붙임으로 작은 따옴표를 문자열을 만드는 문자가 아니라고 인식하게 한다.
