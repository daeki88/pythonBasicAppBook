...
상속은 기존 클래스의 성질을 이어 받아 새로운 클래스를 만드는 방법이다. 기존 클래스를 기반 클래스(base class) 또는 상위 클래스(superclass) 또는 부모 클래스라고 하고 새로운 클래스를 파생 클래스(derived class) 또는 하위 클래스(subclass) 또는 자식 클래스라고 한다. 자식이 부모로부터 재산을 상속받듯이 자식 클래스도 부모 클래스가 가지고 있는 성질들을 물려받는다. 예를 들면, 회사원이라는 클래스가 있으면 회사원은 사람이므로 사람 클래스로부터 상속을 받아서 사람이 갖는 성질을 그대로 가지고 가며 회사원의 특징만 새롭게 추가하여 사용한다. 회사원 중에는 관리자, 일반사원, 시간제 근무자가 있을 수 있다. 이러한 사람들은 회사원의 특징을 물려받고 그렇지 않은 부분만 클래스에 추가하여 사용하면 중복되어 사용되는 부분을 줄일 수 있다.
...
class 회사원(사람):
    ...
    def __init__(self, 이름, 나이, 성별, 월급, 채용일):
        사람.__init__(self, 이름, 나이, 성별)
        self.월급 = 월급
        self.채용일 = 채용일
        
사원 = 회사원("홍길동", 28, "남", 100, 20171101)
사원.누구()
```
